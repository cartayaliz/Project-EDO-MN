...
\and Lianet Tamarit Tejas C211
\and Maya Ramón Castellanos C212}
\date{}
...
\subsection*{Modelo físico y planteo de la EDO}
...
\subsection{Determinación de la Curva $y=f(x)$}
...
\subsection{Cálculo del Radio del Orificio ($r$)}
Regresamos a: 
\[ \pi x^2 (-C) = -\pi r^2 \sqrt{2gy} \]
...
\subsection*{Variación de la pendiente}
...
\section*{Solución:}
\subsection*{1. Determinación de los Puntos de Equilibrio}
...
\subsection*{2. Clasificación de la Estabilidad}
...
\clearpage
\clearpage
\section*{Parte D: Análisis Numérico y Validación}
...
Para el problema asignado (Tema 5: Clepsydra), el diseño de la curva del tanque $y = 4x^4$ fue determinado específicamente para que la velocidad de vaciado $\frac{dy}{dt}$ fuera constante.

\begin{itemize}
    \item Se utiliza la función original $\texttt{f\_clepsydra}$ en los métodos numéricos.
    \item La gráfica resultante muestra una \textbf{línea recta perfecta} desde $y(0)=4 \text{ ft}$ hasta $y(\tau)=0$, donde $\tau$ es el tiempo de vaciado.
\end{itemize}

Esta solución lineal valida que el diseño teórico del reloj de agua es correcto, ya que el nivel del agua desciende a una velocidad constante de $4 \text{ in/h}$.

\begin{figure}[h!]
    \centering
    % INSERTAR: Gráfica de la simulación del vaciado (línea recta)
    \caption{Simulación del vaciado del Reloj de Agua (Clepsydra) usando Euler Mejorado. Se observa la disminución lineal de la altura $y(t)$.}
\end{figure}

\subsection*{2. Análisis de Error, Convergencia y Costo Computacional}

Dado que la solución exacta de la Clepsydra es lineal, el error global de los métodos de Euler Explícito y Mejorado tiende a ser cero. Para cumplir con el requisito de validar el \textbf{orden de convergencia} de los algoritmos, utilizamos un \textbf{problema de referencia (Benchmark)}: el vaciado de un tanque cilíndrico, cuya EDO no es lineal y sí produce errores medibles.

\subsubsection*{Tabla Comparativa de Algoritmos}
...
\begin{figure}[h!]
    \centering
    % INSERTAR: La tabla comparativa final generada por la consola de Python.
    \caption{Tabla Comparativa de Métodos Numéricos (Error y Orden obtenidos usando Benchmark Cilíndrico).}
\end{figure}

\subsubsection*{Orden de Convergencia (Gráfica Log-Log)}

El código prueba varios $\Delta t$ (pasos de tiempo). En la gráfica log-log del error absoluto vs $\Delta t$:
\begin{itemize}
    \item La pendiente de la curva del \textbf{Euler Explícito} es cercana a 1, confirmando su \textbf{Orden 1} de convergencia.
    \item La pendiente de la curva del \textbf{Euler Mejorado} es cercana a 2, confirmando su \textbf{Orden 2} de convergencia.
    \item Esto demuestra que Euler Mejorado es significativamente más preciso, ya que su error disminuye cuadráticamente con la reducción de $\Delta t$.
\end{itemize}

\begin{figure}[h!]
    \centering
    % INSERTAR: Gráfica Log-Log de la convergencia.
    \caption{Gráfica Log-Log de la convergencia para el Benchmark Cilíndrico. La pendiente del método de Orden 2 (Euler Mejorado) es visiblemente más pronunciada.}
\end{figure}
...
\begin{itemize}
    \item Se simula el vaciado del tanque con la altura inicial nominal ($y_0 = 4.0 \text{ ft}$) y una pequeña perturbación ($y_0 = 4.1 \text{ ft}$).
    \item La gráfica muestra la \textbf{diferencia absoluta} entre ambas trayectorias.
\end{itemize}
...
\begin{figure}[h!]
    \centering
    % INSERTAR: Gráfica triple de Sensibilidad (y Estabilidad).
    \caption{Análisis de Sensibilidad (Condición Inicial y Parámetros) y Estabilidad (Pasos Grandes).}
\end{figure}
...
\begin{itemize}
    \item \textbf{Euler Explícito} (Orden 1) es inestable y la solución numérica puede divergir o dar resultados físicamente imposibles al usar un $\Delta t$ muy grande.
    \item \textbf{Euler Mejorado} (Orden 2) mantiene mejor su estabilidad o precisión incluso con pasos grandes, confirmando que es un algoritmo más robusto para este tipo de EDOs.
\end{itemize}
\end{document}